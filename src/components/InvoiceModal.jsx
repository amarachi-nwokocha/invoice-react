...
export default function InvoiceModal({ invoice, onClose }) {
  ...
  const downloadPDF = async () => {
    ...
    pdf.save(`invoice-${invoice?.id || "invoice"}.pdf`);
      ...
      <div className="bg-white w-full max-w-7xl max-h-[95vh] sm:max-h-[90vh] overflow-y-auto rounded-xl sm:rounded-2xl shadow-xl p-4 sm:p-6 md:p-8 relative">
        ...
        <div className="mb-6 sm:mb-8 pr-8 md:flex md:justify-between">
          ...
          <div className="mt-4 flex flex-col sm:flex-row gap-2 sm:gap-3">
            <button
              className="text-[#003EFF] border-2 rounded-full px-4 py-2 hover:bg-gray-50 transition text-sm w-full sm:w-auto"
              onClick={downloadPDF}
            >
              Download As PDF
            </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
          ...
          <div
            ref={pdfRef}
            className="lg:col-span-2 space-y-4 sm:space-y-6 border rounded-xl p-4 sm:p-6 md:p-10"
          >
            {/* Sender + Customer */}
            <div
              className={`p-4 sm:p-6 rounded-2xl sm:rounded-3xl ${getStatusColor(status)}`}
            >
                ...
                <div className="flex-1">
                  ...
                  <div className="flex gap-3 sm:gap-4 items-start">
                    ...
                    <div className="text-xs sm:text-sm space-y-1">
                      ...
                      <p className="text-[#697598] break-all">
                ...
                <table className="hidden sm:table w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-200">
                      ...
                      <th className="text-right py-3 px-3 font-normal text-slate-600">
                        Quantity
                      </th>
                      <th className="text-right py-3 px-3 font-normal text-slate-600">
                        Unit Price
            ...
            <div className="bg-[#F6F8FA] p-3 sm:p-4 rounded-xl">
              <h2 className="text-[#B7BDCF] text-xs sm:text-sm font-medium">
                NOTE
              </h2>
              <p className="text-[#666F77] text-xs sm:text-sm mt-1">
          ...
          {/* SIDEBAR */}
          <div>
            <h2 className="font-semibold mb-4 sm:mb-6 text-lg sm:text-xl">
              Invoice Activity
            </h2>
            <Activity />
          </div>
